...
# <xbar.dependencies>python, simplejson</xbar.dependencies>
...
if status1_key != 'OK' :
    ...
    debug = True
    error = True
    if 'status' in result.keys() and status1_key != 'OK' :
...
try:
    ...
    if status2_key != 'OK' :
        ...
        debug = True
        error = True
        if 'status' in result['rows'][0]['elements'][0].keys() and status2_key != 'OK':
...
###############################################################################################################################################################################################################################################
    print "---"
    try:
        if result['rows'][0]['elements'][0].has_key('duration_in_traffic'):
            driving_time = result['rows'][0]['elements'][0]['duration_in_traffic']['text']
        elif result['rows'][0]['elements'][0].has_key('duration'):
