...
home_path=$HOME/Library/LaunchAgents       # uncomment this line to list User Agents
glob_agent_path=/Library/LaunchAgents      # uncomment this line to list Global Agents
...
if [[ -n "$home_path" ]]
then compgen -G "$home_path"*.plist &> /dev/null && (
    plist=("$home_path"/*.plist)
    listservices "${plist[@]}" )
fi

if [[ -n "$glob_agent_path" ]]
then compgen -G "$glob_agent_path"*.plist &> /dev/null && (
