#!/usr/bin/env bash

# <xbar.title>VPN status/control (via Network Extensions)</xbar.title>
# <xbar.version>v1.1</xbar.version>
# <xbar.author>Andrius Vitkauskas</xbar.author>
# <xbar.author.github>anandi108</xbar.author.github>
# <xbar.desc>Displays status of MacOS VPN interfaces (via Network Extensions) with option to connect/disconnect.</xbar.desc>
# <xbar.image>https://raw.githubusercontent.com/anandi108/xbar_vpn_next/refs/heads/master/img/screenshot.jpg</xbar.image>
# <xbar.abouturl>https://github.com/anandi108/xbar_vpn_next</xbar.abouturl>
# <xbar.dependencies>scutil,bash,date,readlink</xbar.dependencies>

# <xbar.var>boolean(VPN_NEXT__DISPLAY_TYPE_ICON=true): Display the connection type speciffic image for each VPN profile (Generic, IPSEC, WireGuard, AnyConnect)</xbar.var>
# <xbar.var>boolean(VPN_NEXT__DISPLAY_DURATION=true):  Display calculated duration of time connected, instead of the start date/time</xbar.var>
# <xbar.var>boolean(VPN_NEXT__DISPLAY_SECONDS=true):   Display time connected with the seconds precission, instead of minutes precission</xbar.var>
# <xbar.var>string(VPN_NEXT__DATE_FORMAT="default"):   Date display format (unix date util), when displaying start date/time. Defaults to %Y-%m-%d</xbar.var>

# <xbar.var>string(VPN_NEXT__MENU_TEXT="default"):         VPN menu root item text for different states. Expected format: default | "<status_off> <status_on_1> <status_on_multi> <status_progress> [ <dark_status_off> <dark_status_on_1> <dark_status_on_multi> <dark_status_progress> ]" </xbar.var>
# <xbar.var>string(VPN_NEXT__MENU_COUNT="default"):        VPN menu root item active connections count indicator text. Expected format: default | circled | dice | "<count_0> <count_1> <count_2> ... <count_n> <count_more>" </xbar.var>
# <xbar.var>string(VPN_NEXT__MENU_COLOR="default"):        Font colors for VPN menu root item. Expected format: default | "<color_off> <color_on> <color_progress> <dark_color_off> <dark_color_on> <dark_color_progress>". Example: "#555555 #550000 #005500 #999999 #FFFF99 #99FF99"</xbar.var>
# <xbar.var>string(VPN_NEXT__MENU_FONT_NAME="CourierNew"): Font name for VPN menu root item. </xbar.var>
# <xbar.var>number(VPN_NEXT__MENU_FONT_SIZE=14):           Font size for VPN menu root item. </xbar.var>

# <xbar.var>string(VPN_NEXT__ITEM_TEXT="default"):         VPN menu dropdown list item text for different states. Expected format: default | "<item_off> <item_on> <item_progress> [ <dark_item_off> <dark_item_on> <dark_item_progress> ]" </xbar.var>
# <xbar.var>string(VPN_NEXT__ITEM_COLOR="default"):        Font colors for VPN menu dropdown list items. Expected format: default | "<color_off> <color_on> <color_progress> <dark_color_off> <dark_color_on> <dark_color_progress>". Example: "#555555 #550000 #005500 #999999 #FFFF99 #99FF99"</xbar.var>
# <xbar.var>string(VPN_NEXT__ITEM_FONT_NAME="CourierNew"): Font name for VPN menu dropdown list items. </xbar.var>
# <xbar.var>number(VPN_NEXT__ITEM_FONT_SIZE=14):           Font size for VPN menu dropdown list items. </xbar.var>

main() {
    parse_config

    local action="$1"
    shift
    case "$action" in
        install|link|symlink)
            install_symlink "$@"
            ;;
        list|list_vpns)
            list_vpns
            ;;
        text|xbar_text)
            xbar_text "$@"
            ;;
        color|xbar_color)
            xbar_color "$@"
            ;;
        start|stop)
            local vpn="$1"
            if [ -z "${vpn// /}" ]; then
                echo "ERROR: VPN profile name is required" >&2
                return 1
            fi
            
            local vpns="$(list_vpns)"
            local regex="^$vpn|.*$"
            if [[ ! "$vpns" =~ $regex ]]; then
                echo "ERROR: VPN profile '$vpn' doesn't exist" >&2
                return 1
            fi

            scutil --nc $action "$vpn"
            ;;
        *)
            xbar_menu "$@"
            ;;
    esac
}

parse_config() {
    define_symbols
    define_icons

    # Parse params
    #
    DISPLAY_TYPE_ICON="${VPN_NEXT__DISPLAY_TYPE_ICON:-true}"
    DISPLAY_DURATION="${VPN_NEXT__DISPLAY_DURATION:-true}"
    DISPLAY_SECONDS="${VPN_NEXT__DISPLAY_SECONDS:-true}"
    DATE_FORMAT="${VPN_NEXT__DATE_FORMAT#default}"
    #
    MENU_TEXT="${VPN_NEXT__MENU_TEXT#default}"
    MENU_COUNT="${VPN_NEXT__MENU_COUNT}"
    MENU_COLOR="${VPN_NEXT__MENU_COLOR#default}"
    MENU_FONT_NAME="${VPN_NEXT__MENU_FONT_NAME:-CourierNew}"
    MENU_FONT_SIZE="${VPN_NEXT__MENU_FONT_SIZE:-14}"
    #
    ITEM_TEXT="${VPN_NEXT__ITEM_TEXT#default}"
    ITEM_COLOR="${VPN_NEXT__ITEM_COLOR#default}"
    ITEM_FONT_NAME="${VPN_NEXT__ITEM_FONT_NAME:-CourierNew}"
    ITEM_FONT_SIZE="${VPN_NEXT__ITEM_FONT_SIZE:-14}"
    #
    local default_colors="#555555 #550000 #005500 #999999 #FFFF99 #99FF99"
    local default_menu_text="$SYMBOL_UNLOCKED $SYMBOL_LOCKED $SYMBOL_LOCKED $SYMBOL_HOURGLASS_FULL"
    local default_item_text="$SYMBOL_BOXBOX $SYMBOL_LOCKED $SYMBOL_HOURGLASS_COLOR"
    
    # Preprocess params
    #
    MENU_COLOR="$( echo $MENU_COLOR )"
    MENU_COLOR=( $( light_dark_subset 3 ${MENU_COLOR:-"$default_colors"} ) )
    #
    ITEM_COLOR="$( echo $ITEM_COLOR )"
    ITEM_COLOR=( $( light_dark_subset 3 ${ITEM_COLOR:-"$default_colors"} ) )
    #
    MENU_TEXT="$( echo $MENU_TEXT )"
    MENU_TEXT=( $(light_dark_subset 4 ${MENU_TEXT:-"$default_menu_text"} ) )
    #
    ITEM_TEXT="$( echo $ITEM_TEXT )"
    ITEM_TEXT=( $(light_dark_subset 3 ${ITEM_TEXT:-"$default_item_text"} ) )
    #
    case "$MENU_COUNT" in
        '')      MENU_COUNT=() ;;
        default) MENU_COUNT=( '' {1..9} $SYMBOL_ELIPSIS ) ;;
        circled) MENU_COUNT=( '' $SYMBOL_CIRCLED_{1..9} $SYMBOL_ELIPSIS ) ;;
        dice)    MENU_COUNT=( '' $SYMBOL_DICE_{1..6} $SYMBOL_ELIPSIS ) ;;
        *)       MENU_COUNT=( $MENU_COUNT ) ;;
    esac
    #
    DATE_FORMAT="${DATE_FORMAT/default/}"
}

light_dark_subset() {
    local size="${1}"
    shift 1
    if [[ "$XBARDarkMode" == "true" && $# -gt 0$size ]]; then
        echo "${@:((size+1)):size}"
    else
        echo "${@:1:size}"
    fi
}

install_symlink() {
    local basepath="$HOME/Library/Application Support/xbar/plugins/vpn-next"

    if [[ "$1" =~ ^-l|--list|list|ls$ ]]; then
        cd "$(dirname "$basepath")"
        ls -l "$(basename "$basepath")"*.sh
        return $r
    fi

    local refresh="${1:-2s}"
    for file in "$basepath"*.sh; do 
        if [ -s "$file" ]; then
            echo "Removing   '${file##*/}'"
            rm -f "$file" 
        fi
    done

    local this="$(readlink -f ${BASH_SOURCE})"
    echo "Installing '$(basename "$basepath").${refresh}.sh' -> '$this'"
    ln -s "$this" "${basepath}.${refresh}.sh"
}

list_vpns() {
    local regex='\* \(([^\)]*)\).*"([^"]*)" *(\[.*\])'
    scutil --nc list | 
    while read line; do
        if [[ "$line" =~ $regex ]]; then 
            echo "${BASH_REMATCH[2]}:${BASH_REMATCH[3]}:${BASH_REMATCH[1]}"
        fi
    done
}

xbar_text() {
    local count="${1:-0}"
    local progress="${2:-0}"

    local count_index="$count"
    local count_size=${#MENU_COUNT[@]}
    if [[ $count_size -gt 0 && 0$count_index -ge $count_size ]]; then
        count_index=$((count_size-1))
    fi

    case "$count" in
        [01])
            local status_text="${MENU_TEXT[$count]}"
            local count_text="${MENU_COUNT[$count_index]}"
            ;;
        [0-9]*)
            local status_text="${MENU_TEXT[2]}"
            local count_text="${MENU_COUNT[$count_index]}"
            ;;
        *)
            local status_text="${MENU_TEXT[0]}"
            local count_text="?"
            ;;
    esac

    case "$progress" in
        0|'')   local progress_text="" ;;
        [0-9]*) local progress_text="${MENU_TEXT[3]}" ;;
        *)      local progress_text="" ;;
    esac

    echo "${status_text//_/ }${count_text//_/ }${progress_text//_/ }"
}

xbar_color() {
    local count="${1:-0}"
    local progress="${2:-0}"

    local index=0
    if [ 0$progress -ge 1 ]; then
        index=2
    elif [ 0$count -gt 0 ]; then
        index=1
    fi

    echo "${MENU_COLOR[$index]}"
}

format_connected_time() {
    local from="${1:-$(date)}"

    local date_format="${DATE_FORMAT:-%Y-%m-%d}"
    if [ "${DISPLAY_SECONDS}" == "true" ]; then
        local time_format='%H:%M:%S'
    else
        local time_format='%H:%M'
    fi

    if date --version >/dev/null 2>&1 ; then
        # GNU
        if [ "$DISPLAY_DURATION" == "true" ]; then
            local sec="$(($(date +%s) - $(date -d "$from" +%s)))"
            date -u -d @"$sec" "+$time_format"
        else
            date -d "$from" "+$date_format $time_format"
        fi
    else
        # BSD
        if [ "$DISPLAY_DURATION" == "true" ]; then
            local sec="$(($(date +%s) - $(date -jf'%m/%d/%Y %H:%M:%S' "$from" +%s)))"
            date -u -r "$sec" "+$time_format"
        else
            date -jf"%m/%d/%Y %T" "$from" "+$date_format $time_format"
        fi
    fi
}

xbar_menu() {
    local items=()
    local count=0
    local progress=0
    local line=''

    list_vpns | {
        while read line; do
            if [[ ! "$line" =~ (.*):\[(.*)\]:(.*) ]]; then
                echo "ERROR: wrong list_vpns() output - '$line'" >&2
                next
            fi
            local name="${BASH_REMATCH[1]}"
            local text=""
            local color="${ITEM_COLOR[0]}"
            local type="${BASH_REMATCH[2]}"
            local status="${BASH_REMATCH[3]}"
            local command="bash='$0'"
            local params="terminal=false refresh=true font=${ITEM_FONT_NAME:-CourierNew} size=${ITEM_FONT_SIZE:-14}"

            local type_img="$ICON_VPN"
            case "$type" in
                IPSec) type_img="$ICON_CISCO" ;;
                # PPP:L2TP) type_img="$ICON_L2TP"  ;;
                VPN:com.wireguard.macos)  type_img="$ICON_WIREGUARD"  ;;
                VPN:com.cisco.anyconnect) type_img="$ICON_ANYCONNECT" ;;
            esac
            if [[ "$DISPLAY_TYPE_ICON" == "true" && -n "${type_img// /}" ]]; then
                params+=" image=${type_img}"
            fi

            case "$status" in
                Disconnected)
                    text="${ITEM_TEXT[0]//_/ }"
                    color="${ITEM_COLOR[0]}"
                    command+=" param1=start param2='${name}'"
                    ;;
                Connected)
                    text="${ITEM_TEXT[1]//_/ }"
                    local data="$(scutil --nc status "$name")"
                    if [[ "$data" =~ 'LastStatusChangeTime : '([0-9:/ ]*) ]]; then
                        text+=" $( format_connected_time "${BASH_REMATCH[1]}" )"
                    fi
                    color="${ITEM_COLOR[1]}"
                    command+=" param1=stop param2='${name}'"
                    ((count++))
                    ;;
                Connecting)
                    text="${ITEM_TEXT[2]//_/ }"
                    color="${ITEM_COLOR[2]}"
                    command+=" param1=stop param2='${name}'"
                    ((progress++))
                    ;;
                *)
                    text="${ITEM_TEXT[2]} $status"
                    color="${ITEM_COLOR[2]}"
                    command+=" param1=stop param2='${name}'"
                    ((progress++))
                    ;;
            esac
            items+=(
                "${name}:  ${text} | ${command} color=${color} $params"
            )
        done

        echo -e "$(xbar_text "$count" "$progress") | color=$(xbar_color "$count" "$progress") font=${MENU_FONT_NAME:-CourierNew} size=${MENU_FONT_SIZE:-14}"
        echo "---"
        for item in "${items[@]}"; do
            echo -e "$item"
        done
    }
}

define_symbols() {
    SYMBOL_UNLOCKED='\xf0\x9f\x94\x93' # 🔓
    SYMBOL_LOCKED='\xf0\x9f\x94\x90'   # 🔐
    SYMBOL_KEY='\xf0\x9f\x94\x91'      # 🔑
    SYMBOL_LINK='\xf0\x9f\x94\x97'     # 🔗
    SYMBOL_BOXBOX='\xe2\xa7\x89'       # ⧉
    SYMBOL_ELIPSIS='\xe2\x80\xa6'      # …

    SYMBOL_HOURGLASS_COLOR='\xe2\x8f\xb3' # ⏳
    SYMBOL_HOURGLASS_EMPTY='\xe2\xa7\x96' # ⧖
    SYMBOL_HOURGLASS_FULL='\xe2\xa7\x97'  # ⧗

    SYMBOL_PLUG='\xf0\x9f\x94\x8c'     # 🔌
    SYMBOL_CASTLE='\xe2\x9b\xab'       # ⛫
    SYMBOL_SHIELD_EMPTY='\xe2\x9b\x89' # ⛉
    SYMBOL_SHIELD_CROSS='\xe2\x9b\xa8' # ⛨
    SYMBOL_SHIELD_FULL='\xe2\x9b\x8a'  # ⛊
    SYMBOL_SHIELD_COLOR='\xf0\x9f\x9b\xa1\xef\xb8\x8f' # 🛡️

    SYMBOL_CIRCLED_1='\xe2\x9e\x80'  # ➀
    SYMBOL_CIRCLED_2='\xe2\x9e\x81'  # ➁
    SYMBOL_CIRCLED_3='\xe2\x9e\x82'  # ➂
    SYMBOL_CIRCLED_4='\xe2\x9e\x83'  # ➃
    SYMBOL_CIRCLED_5='\xe2\x9e\x84'  # ➄
    SYMBOL_CIRCLED_6='\xe2\x9e\x85'  # ➅
    SYMBOL_CIRCLED_7='\xe2\x9e\x86'  # ➆
    SYMBOL_CIRCLED_8='\xe2\x9e\x87'  # ➇
    SYMBOL_CIRCLED_9='\xe2\x9e\x88'  # ➈
    SYMBOL_CIRCLED_10='\xe2\x9e\x89' # ➉

    SYMBOL_DICE_1='\xe2\x9a\x80' # ⚀
    SYMBOL_DICE_2='\xe2\x9a\x81' # ⚁
    SYMBOL_DICE_3='\xe2\x9a\x82' # ⚂
    SYMBOL_DICE_4='\xe2\x9a\x83' # ⚃
    SYMBOL_DICE_5='\xe2\x9a\x84' # ⚄
    SYMBOL_DICE_6='\xe2\x9a\x85' # ⚅
}

define_icons() {
    ICON_VPN=''
    ICON_L2TP=''
    ICON_CISCO=''
    ICON_WIREGUARD=''
    ICON_ANYCONNECT=''
}

main "$@"
