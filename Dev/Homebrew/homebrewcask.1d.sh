...
brewupdate=$(/usr/local/bin/brew update) # Plugin performs 'brew update' and captures output. Currently does nothing with output. Use this variable to add further information to plugin (e.g. new non-cask formulae which have been updated)

brewcasklist=$(/usr/local/bin/brew cask ls -1 | sed 's/(!)//g' | xargs /usr/local/bin/brew cask info | grep -A 1 'Not installed' | sed -e 's_Not installed__g' -e 's_https://github\.com/caskroom/homebrew-cask/blob/master/Casks/__g' -e 's_\.rb__g')
...
for line in $brewcasklist; do echo $line | grep "[a-z]" | sed 's/\(.*\)/Update & | bash=\"brew cask install &\" terminal=true refresh=/g' ; done
echo "---"
