...
IMAGE_SIZES=$(echo ${DOCKER_DF} | cut -d " " -f 1 | dehumanise)
CONTAINER_SIZES=$(echo ${DOCKER_DF} | cut -d " " -f 2 | dehumanise)
VOLUME_SIZES=$(echo ${DOCKER_DF} | cut -d " " -f 3 | dehumanise)
BUILD_SIZES=$(echo ${DOCKER_DF} | cut -d " " -f 4 | dehumanise)

TOTAL=$((IMAGE_SIZES + CONTAINER_SIZES + VOLUME_SIZES + BUILD_SIZES))

echo "🐳: $(humanize ${TOTAL})"
echo "---"
echo "images: $(echo ${DOCKER_DF} | cut -d " " -f 1)"
echo "containers: $(echo ${DOCKER_DF} | cut -d " " -f 2)"
echo "volumes: $(echo ${DOCKER_DF} | cut -d " " -f 3)"
echo "build cache: $(echo ${DOCKER_DF} | cut -d " " -f 4)"
echo "---"
