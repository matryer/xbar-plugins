#!/usr/bin/env python3

# <xbar.title>Blackcoin Ticker ($1USD)</xbar.title>
# <xbar.version>v1.1</xbar.version>
# <xbar.author>impshum</xbar.author>
# <xbar.author.github>impshum</xbar.author.github>
# <xbar.desc>Displays current Blackcoin price for $1 from Coinmarketcap</xbar.desc>
# <xbar.image>https://i.imgur.com/PjuyXQd.jpg</xbar.image>
#
# by impshum

import json
from urllib.request import Request, urlopen

API_KEY = ''

headers = {'X-CMC_PRO_API_KEY': API_KEY, 'Accept': 'application/json'}
req = Request('https://pro-api.coinmarketcap.com/v2/cryptocurrency/quotes/latest?symbol=BLK', None, headers)
data = urlopen(req).read()
result = json.loads(data.decode('utf-8'))

usd = result['data']['BLK'][0]['quote']['USD']

def flow():
    if usd['volume_change_24h'] > 0:
        print((' $%.4f | image=' % float(usd['price'])))
    else:
        print((' $%.4f | image=' % float(usd['price'])))

flow()
