...
bit_price=$(curl -s "https://api.coinbase.com/v2/prices/BTC-USD/buy" -H "CB-VERSION: 2017-04-16" | egrep -o '{"data":{"amount":"[0-9]+(\.)?([0-9]{0,2}")?' | sed 's/{"data":{"amount"://'  | sed 's:^.\(.*\).$:\1:')
eth_price=$(curl -s "https://api.coinbase.com/v2/prices/ETH-USD/buy" -H "CB-VERSION: 2017-04-16" | egrep -o '{"data":{"amount":"[0-9]+(\.)?([0-9]{0,2}")?' | sed 's/{"data":{"amount"://'  | sed 's:^.\(.*\).$:\1:')
ltc_price=$(curl -s "https://api.coinbase.com/v2/prices/LTC-USD/buy" -H "CB-VERSION: 2017-04-16" | egrep -o '{"data":{"amount":"[0-9]+(\.)?([0-9]{0,2}")?' | sed 's/{"data":{"amount"://'  | sed 's:^.\(.*\).$:\1:')
