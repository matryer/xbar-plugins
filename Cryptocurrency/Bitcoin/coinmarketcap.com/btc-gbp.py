#!/usr/bin/env python3

# <xbar.title>Bitcoin coin Ticker (£1GBP)</xbar.title>
# <xbar.version>v1.1</xbar.version>
# <xbar.author>impshum</xbar.author>
# <xbar.author.github>impshum</xbar.author.github>
# <xbar.desc>Displays current Bitcoin price for £1 from Coinmarketcap</xbar.desc>
# <xbar.image>https://i.imgur.com/oXZWn0I.jpg</xbar.image>
#
# by impshum

import json
from urllib.request import Request, urlopen

API_KEY = ''

headers = {'X-CMC_PRO_API_KEY': API_KEY, 'Accept': 'application/json'}
req = Request('https://pro-api.coinmarketcap.com/v2/cryptocurrency/quotes/latest?symbol=BTC&convert=GBP', None, headers)
data = urlopen(req).read()
result = json.loads(data.decode('utf-8'))

gbp = result['data']['BTC'][0]['quote']['GBP']

def flow():
    if gbp['volume_change_24h'] > 0:
        print((' £%.4f | image=' % float(gbp['price'])))
    else:
        print((' £%.4f | image=' % float(gbp['price'])))

flow()
