#!/usr/bin/env bash
#
# Unsplash Background
# -------------------
# Periodically change all desktops/spaces
# background to a random unsplash photo
#
# <xbar.title>Unsplash Background</xbar.title>
# <xbar.version>v1.0</xbar.version>
# <xbar.author>Marko Radak</xbar.author>
# <xbar.author.github>iammarkoradak</xbar.author.github>
# <xbar.desc>Periodically change all desktops/spaces background to a random unsplash photo.</xbar.desc>

echo '|templateImage='
echo '---'
echo 'Next Image | refresh=true'

curl -L --silent -o /tmp/.background.png "https://unsplash.it/2560/1600/?random"

sqlite3 ~/Library/Application\ Support/Dock/desktoppicture.db "update data set value = '/tmp/.background.png'"

killall Dock
