#!/usr/bin/env python
# coding=utf-8
#
# <bitbar.title>Slack Notification</bitbar.title>
# <bitbar.version>v1.0</bitbar.version>
# <bitbar.author>mgjo5899</bitbar.author>
# <bitbar.author.github>mgjo5899</bitbar.author.github>
# <bitbar.desc>Displays number of unread Slack messages</bitbar.desc>
# <bitbar.image>https://i.imgur.com/I3MdNmU.png</bitbar.image>
# <bitbar.dependencies>python</bitbar.dependencies>
#
# by mgjo5899

try:
	import requests
except ImportError:
	print("You need to `pip install requests`")

import json
from time import sleep
from multiprocessing import Pool

channels_list_url = 'https://slack.com/api/channels.list'
channels_info_url = 'https://slack.com/api/channels.info'

def get_unread_count(token,channel_id):
	header = {'Authorization':'Bearer ' + token}
	data = {'channel':channel_id}
	r = requests.post(channels_info_url, headers=header, data=data)

	try:
		channel_info = json.loads(r.text)['channel']
	except KeyError:
		# Rate limit exceeded
		print(r.text)
		return 0
	
	if 'unread_count' in channel_info.keys():
		return channel_info['unread_count']

	return 0

# Users need to modify this part by generating their own tokens
tokens = ['xoxp-slack-token-place-holder']

if tokens[0] == 'xoxp-slack-token-place-holder':
	print('Configure Slack tokens first')
	exit(1)

channels = {}
results = []
unread_count = 0
pool = Pool(processes=len(tokens))

for token in tokens:
	header = {'Authorization':'Bearer ' + token}
	channels[token] = []
	r = requests.post(channels_list_url, headers=header)
	sleep(1)
	result = json.loads(r.text)['channels']

	for j in range(len(result)):
		channel_id = result[j]['id']
		channels[token].append(channel_id)

for token in tokens:
	for channel_id in channels[token]:
		result = pool.apply_async(get_unread_count, (token, channel_id))
		results.append(result)

for r in results:
	unread_count += r.get()

# THIS IS A SPECIAL FEATURE FOR USERS WITH DARK MENU BAR ENABLED
# REMOVE COMMENTING OUT TO USE
#if unread_count == 0:
#	print("| image=")
#else:
#	print("| image=")
print(str(unread_count) + " | image=")
