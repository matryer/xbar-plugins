...
echo $xmldata | grep "Video" > /dev/null 2>&1
if [ $? -ne 0 ]; then
  echo "○"
  echo "---"
...
if [ echo "$xmldata" | grep "movie" > /dev/null 2>&1 -eq 0 ]; then
  ...
  keydata=$(echo "$xmldata" | /usr/local/bin/sed -e 's/" /\n/g' | /usr/bin/grep "key" | head -n1 | cut -f2 -d "\"" | sed -e 's/\//%2F/g' | /usr/bin/head -n1)
fi
...
#/usr/local/bin/mogrify -scale 75x100 $tmpfile
/usr/local/bin/mogrify -scale 75x100 /tmp/image.jpg >> /tmp/output.txt 2>&1

# Encode our image into base64 text for BitBar
imgstr=$(/usr/bin/base64 /tmp/image.jpg)
