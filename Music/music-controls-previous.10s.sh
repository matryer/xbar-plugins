#!/bin/bash

# Plays the previous track in cmus, iTunes or Spotify.
#
# Based on cmus script by mcchrish
# and Spotify script by alekseysotnikov
#
# metadata
# <bitbar.title>Music Controls - Previous Track Button</bitbar.title>
# <bitbar.version>v1.0</bitbar.version>
# <bitbar.author>Sebastián Barschkis</bitbar.author>
# <bitbar.author.github>sebbas</bitbar.author.github>
# <bitbar.desc>Plays the previous track in cmus, iTunes or Spotify.</bitbar.desc>
# <bitbar.image>https://user-images.githubusercontent.com/1890600/54347955-56d68600-4648-11e9-8721-9dc087187be4.png</bitbar.image>

export PATH="/usr/local/bin:/usr/bin:/bin:$PATH"

# Trigger previous track in music application
if [[ "$1" = 'cmus_prev' ]]; then
  cmus-remote --prev
  exit
fi
if [[ "$1" = 'itunes_prev' ]]; then
  osascript -e 'tell application "iTunes" to previous track'
  exit
fi
if [[ "$1" = 'spotify_prev' ]]; then
  osascript -e 'tell application "Spotify" to previous track'
  exit
fi

CMUS="cmus"
ITUNES="iTunes"
SPOTIFY="Spotify"
PIANOBAR="pianobar"

prev_icon_light=""

prev_icon_dark=""

BitBarDarkMode=${BitBarDarkMode}
current_source=0

# Get pid of music apps to see if they are currently running
cmus_pid=$(pgrep -x $CMUS)
itunes_pid=$(pgrep -x $ITUNES)
spotify_pid=$(pgrep -x $SPOTIFY)
pianobar_pid=$(pgrep -x $PIANOBAR)

# Keep track of music source
# Reorder items in for -loop to your liking to change order of precendece
# (i.e. if available, left-most audio source will be used first)
for s in $CMUS $ITUNES $SPOTIFY $PIANOBAR; do
  if [[ $s = $CMUS && $cmus_pid -gt 0 ]]; then
    current_source=$CMUS
    break
  elif [[ $s = $ITUNES && $itunes_pid -gt 0 ]]; then
    current_source=$ITUNES
    break
  elif [[ $s = $SPOTIFY && $spotify_pid -gt 0 ]]; then
    current_source=$SPOTIFY
    break
  elif [[ $s = $PIANOBAR && $pianobar_pid -gt 0 ]]; then
    # pianobar does not support previous song trigger
    exit
  fi
done

# Set previous track icon based on dark mode setup and trigger previous track in correct music app
if [[ "$BitBarDarkMode" ]]; then
  if [[ $current_source = $CMUS ]]; then
    echo " | image=$prev_icon_dark bash='$0' param1='cmus_prev' terminal=false refresh=false"
  elif [[ $current_source = $ITUNES ]]; then
    echo " | image=$prev_icon_dark bash='$0' param1='itunes_prev' terminal=false refresh=false"
  elif [[ $current_source = $SPOTIFY ]]; then
    echo " | image=$prev_icon_dark bash='$0' param1='spotify_prev' terminal=false refresh=false"
  fi
else
  if [[ $current_source = $CMUS ]]; then
    echo " | image=$prev_icon_light bash='$0' param1='cmus_prev' terminal=false refresh=false"
  elif [[ $current_source = $ITUNES ]]; then
    echo " | image=$prev_icon_light bash='$0' param1='itunes_prev' terminal=false refresh=false"
  elif [[ $current_source = $SPOTIFY ]]; then
    echo " | image=$prev_icon_light bash='$0' param1='spotify_prev' terminal=false refresh=false"
  fi
fi
